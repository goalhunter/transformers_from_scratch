...
def build_transformer(src_vocab_size:int, tgt_vocab_size:int, src_len:int, tgt_len:int, d_model:int = 512, N:int = 6, h:int = 8, dropout:float = 0.1, d_ff:int = 2048) -> Transformer:
    # embedding layers
    ...
    encoder_blocks = []
    for _ in range(N):
        encoder_self_attention_block = MultiHeadAttentionBlock(d_model, n_heads=h, dropout=dropout)
    ...
    decoder_blocks = []
    for _ in range(N):
        decoder_self_attention_block = MultiHeadAttentionBlock(d_model, n_heads=h, dropout=dropout)
        decoder_cross_attention_block = MultiHeadAttentionBlock(d_model, n_heads=h, dropout=dropout)
        feed_forward_block = FeedForwardBlock(d_model, d_ff, dropout)
